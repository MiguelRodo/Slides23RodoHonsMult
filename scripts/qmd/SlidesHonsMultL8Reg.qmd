...
- Simple regression: one predictor, one response
...
- **Linearity**: $\mathrm{E}[Y] = \beta_0 + \beta_1 X_1 + \ldots + \beta_p X_p$
...
$$
\hat{\symbf{\mathcal{B}}} = (\symbf{X}'\symbf{X})^{-1}\symbf{X}'\symbf{Y}.
$$
...
- For example, suppose that we model BMI and blood pressure against age, weight and height:
  ...
  - The simultaneous hypothesis tests will be the same when the considering only groups of coefficients with respect to a particular response variable. As an example, the p-value for the hypothesis that neither age nor weight has an effect on BMI will be the same in both models.
...
\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{_raw/img/2024/pcr_vs_ridge.png}
\end{figure}
...
- However, instead of choosing the linear combinations to maximise the variance, it chooses them to maximise (something like) the covariance of the resultant variables with the response.

# Partial least squares algorithm [@hastie_etal09]

\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{_raw/img/2024/pls_alg.png}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{_raw/img/2024/pcr_vs_pls.png}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{_raw/img/2024/cv-pcr_and_pls.png}
\end{figure}
