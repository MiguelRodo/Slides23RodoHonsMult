...
- A similar argument shows that for $i\ge 2$, the maximum variance $\lambda_i$ is attained when $\mathbf{a}_i=\mathbf{e}_i$ under the constraint of orthogonality with the previous eigenvectors.
  - This constraint can always be satifised as the eigenvectors of a symmetric matrix can be chosen orthogonal.
...
- Visualize the first principal component as an axis along the vector $\mathbf{a} = [1/\sqrt{2}, 1/\sqrt{2}]$, with the projection of the point $[1.2, 0.3]$ onto this axis forming the new variable:
